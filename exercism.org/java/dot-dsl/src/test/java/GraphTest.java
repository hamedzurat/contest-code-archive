...
public class GraphTest {
    ...
    public void testGraphWithAttributes() {
        Graph graph = new Graph(Map.of("foo", "1", "title", "Testing Attrs", "bar", "true")).node("a", Map.of("color", "green"))
                                                                                            .node("c")
                                                                                            .node("b", Map.of("label", "Beta!"))
                                                                                            .edge("b", "c")
                                                                                            .edge("a", "b", Map.of("color", "blue"));

        assertThat(graph.getNodes()).containsExactlyInAnyOrder(new Node("a", Map.of("color", "green")), new Node("c"), new Node("b", Map.of("label", "Beta!")));

        assertThat(graph.getEdges()).containsExactlyInAnyOrder(new Edge("a", "b", Map.of("color", "blue")), new Edge("b", "c"));

        assertThat(graph.getAttributes()).containsExactlyInAnyOrderEntriesOf(Map.of("foo", "1", "title", "Testing Attrs", "bar", "true"));
    }
}
